            ...
            <FormContainer onSubmit={(e) => handleSubmit(e)}>
                {filteredSelectedSeats?.reduce((acc, item, i) => {
                    acc.push(<InputItem key={item?.id}>
                        ...
                        <input data-test="client-cpf" type="text" minLength={11} value={(hashNameCpf[item?.id]?.cpf || " ").trim()} onChange={e => includeCpf(e.target.value, item?.id)} placeholder="Digite seu CPF..." pattern="\d{11}" title="Por favor, digite um cpf valido contendo 11 digitos numéricos"required />
                    </InputItem>)
                    return acc
                }, [])}
